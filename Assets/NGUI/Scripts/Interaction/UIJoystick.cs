...
public class UIJoystick : MonoBehaviour
{
    ...
    IEnumerator fadeOutJoystick()
    {
        ...
        foreach (UIWidget widget in widgetsToFade)
        {
            ...
            //			TweenColor.Begin(widget.gameObject, 0.5f, newColor).method = Tweener.Method.EaseOut;
        }
    }
        ...
        if (target != null)
        {
            ...
            if (pressed)
            {
                ...
                if (centerOnPress)
                {
                    ...
                    foreach (UIWidget widget in widgetsToFade)
                    {
                        //						TweenColor.Begin(widget.gameObject, 0.1f, Color.white).method = Tweener.Method.EaseIn;
                    }
    ...
    private void Move()
    {
        ...
        //Debug.Log("offset.x " +  offset.x + " and offset.y = " + offset.y);

        //Add movement scripts here
        if (offset.y != 0f && mPressed == true)
        {
            Debug.Log("Move Player = " + offset.y);
            //_myCharController.SimpleMove(playerTransform.TransformDirection(Vector3.forward) * (offset.y * 10) * moveSpeed);

            //animation.CrossFade("run");
        }

        if (offset.x != 0f && mPressed == true)
        {
            //Debug.Log("Rotate Player = " + offset.x);
            //playerTransform.Rotate(0, offset.x * Time.deltaTime * rotateSpeed, 0);
        }
